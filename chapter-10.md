...
## Blockchain-based tokens <a href="#blockchain-based-tokens" id="blockchain-based-tokens"></a>
...
### Why was there a craze for NFTs? <a href="#why-was-there-a-craze-for-nfts" id="why-was-there-a-craze-for-nfts"></a>
...
### Plain english mechanics of creating an NFT <a href="#plain-english-mechanics-of-creating-an-nft" id="plain-english-mechanics-of-creating-an-nft"></a>
...
### Exercise: Hands-on learning <a href="#exercise-hands-on-learning" id="exercise-hands-on-learning"></a>
...
### Uses of NFTs <a href="#uses-of-nfts" id="uses-of-nfts"></a>
...
## Minting & managing fungible tokens – Simulating fiat-backed stablecoins <a href="#minting-and-managing-fungible-tokens-simulating-fiat-backed-stablecoins" id="minting-and-managing-fungible-tokens-simulating-fiat-backed-stablecoins"></a>
...
### How do fiat-backed stablecoins provided by Circle & Tether work? <a href="#how-do-fiat-backed-stablecoins-provided-by-circle-and-tether-work" id="how-do-fiat-backed-stablecoins-provided-by-circle-and-tether-work"></a>
...
### Exercise: Simulating fiat-backed stablecoins <a href="#exercise-simulating-fiat-backed-stablecoins" id="exercise-simulating-fiat-backed-stablecoins"></a>
...
| <p><strong>Step 1: “USD Deposit” and On-Chain Minting (Issuer → User)</strong><br>In this step, Account 1 (0x5B38Da6a701c568545dCfcB03FcB875f56beddC4) acts as the issuer. Account 2 (0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2) acts as the customer who sends USD.<br>A. We will begin by pretending that Account 2 has sent $200 to the issuer via traditional means. Assume that has already been done.<br>B. Account 1 (the issuer) mints tokens on-chain. Carry out those actions as described below.<br>    a. Keep Account 1 selected (issuer).<br>    b. Call the function mintForUSDDeposit with: to: Account 2 (0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2) amountInUSD: <code>200</code><br>    This creates 200 sUSDC for Account 2. sUSDC is simulated stablecoin pegged to the US dollar.<br>C. Check the balance<br>    Call: <code>balanceOf(Account 2)</code><br>   (Account 2: 0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2)<br>    You should see: <code>200000000000000000000</code><br>    Note that each sUSDC is broken up into 10*18 units, which is why you are seeing a balance of 200 * 10^18 units for Account 2.<br><br><strong>Step 2: User Transfers sUSDC On-Chain</strong><br>Now Account 2 uses the stablecoin.<br>A. Switch to Account 2 (0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2).<br>B. Call the transfer function with:<br>    a. recipient: Account 3 (0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db)<br>    b. amount: <code>50000000000000000000</code> (50 sUSDC)<br>C. Check balances:<br>    Account 2 (0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2) now has 150 sUSDC<br>    Account 3 (0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db) now has 50 sUSDC<br>This illustrates how users can freely use stablecoins.<br><br><strong>Step 3: User Returns sUSDC to Issuer and Redeems USD</strong><br>Account 3 wants to redeem 30 sUSDC for real USD.<br>A. Account 3 (sends 30 tokens back to the issuer (Account 1).<br>    a. Switch to Account 3 (0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db)<br>    b. Call the transfer function with:<br>     recipient: Account 1 (i.e., 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4)<br>     amount: <code>30000000000000000000</code> (30 sUSDC)<br>B. User requests a USD withdrawal from the issuer via traditional means (we just pretend here).<br>    a. Now Account 1 checks its wallet and sees that Account 3 returned 30 tokens (Account 1 is able to see it has received 30 tokens but determining where they have come from is a somewhat complex off-chain process, which we do not need to get into. However, if you remember what we had done when introducing Ethereum – specifically Part 2, we had seen one or more internal transactions with descriptions of who had sent the ETH to whom. All of that was done using an off-chain process).<br>C. Issuer burns the tokens<br>    a. Switch to Account 1 (0x5B38Da6a701c568545dCfcB03FcB875f56beddC4)<br>    b. Call burnForUSDWithdrawal with:<br>     amountInTokens: <code>30000000000000000000</code><br>     This removes 30 sUSDC from circulation.<br>D. Off-chain settlement<br>     The issuer (Account 1) would now send $30 USD back to Account 3 (we just pretend here).<br><br><strong>Step 4: Delegated Spending</strong><br>This simulates a situation where a parent approves spending of a certain amount by their kid.<br>We will simulate Account 2 approving Account 4 to spend 20 sUSDC.<br>A. Account 2 approves Account 4<br>    a. Switch to Account 2 (0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2)<br>    b. Call the approve function with:<br>     spender: Account 4 (0x78731D3Ca6b7E34aC0F824c42a7cC18A495cabaB)<br>     amount: <code>20000000000000000000</code><br>B. Account 4 transfers sUSDC from Account 2 → Account 5<br>    a. Switch to Account 4<br>    b. Call the transferFrom function with:<br>     sender: Account 2 (0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2)<br>     recipient: Account 5 (0x617F2E2fD72FD9D5503197092aC168c91465E7f2)<br>     amount: <code>20000000000000000000</code><br>Balances will update accordingly.</p> |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
